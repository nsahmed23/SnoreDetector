...
export function useAudioMonitor(
  ...
  // Track `isCurrentlySnoring` synchronously to avoid costly Web Audio API teardown/restarts
      ...
      navigator.mediaDevices.getUserMedia({ audio: true })
        .then((stream) => {
          ...
          const updateVolume = () => {
            ...
            // Loop fusion: Calculate total sum, low frequency sum, and high frequency sum in a single pass.
            // This reduces the iteration overhead inside the 60fps requestAnimationFrame loop.
            let sum = 0;
            let lowFreqSum = 0;
            let highFreqSum = 0;
  ...
  }, [isTracking, thresholdDB, sensitivity]); // Removed `isCurrentlySnoring` to avoid stream restart on state change
