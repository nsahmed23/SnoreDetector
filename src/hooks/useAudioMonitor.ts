...
export function useAudioMonitor(
      ...
      navigator.mediaDevices.getUserMedia({ audio: true })
        .then((stream) => {
          ...
          const updateVolume = () => {
            ...
            // Loop fusion: Compute total sum, low frequency sum, and high frequency sum in a single pass
            let sum = 0;
            let lowFreqSum = 0;
            let highFreqSum = 0;
  ...
  }, [isTracking, thresholdDB, sensitivity]); // Removed isCurrentlySnoring to prevent audio context teardowns
