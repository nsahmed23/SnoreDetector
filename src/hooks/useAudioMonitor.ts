...
export function useAudioMonitor(
  ...
  // Use a ref to track this state synchronously to avoid tearing down the audio stream
      ...
      navigator.mediaDevices.getUserMedia({ audio: true })
        .then((stream) => {
          ...
          const updateVolume = () => {
            ...
            // Fused loop: calculate sum, low frequency sum, and high frequency sum in a single pass
            // This reduces the number of iterations over the data array, improving performance in the 60fps loop.
            let sum = 0;
            let lowFreqSum = 0;
            let highFreqSum = 0;
  ...
  }, [isTracking, thresholdDB, sensitivity]); // Removed isCurrentlySnoring to avoid restarting stream
